...
const AttendanceSystem: React.FC = () => {
 ...
 const searchParams = useSearchParams();
  ...
  const captureAndVerify = async () => {
    ...
    try {
      const res = await fetch("https://myrmidons-pinequest-production.up.railway.app/login", {
        method: "POST",
  ...
  const getCurrentDate = () => {
    const now = new Date()
    return now.toLocaleDateString("mn-MN", {
      month: "numeric",
      day: "numeric",
      year: "numeric",
    })
        ...
        <div className="max-w-2xl mx-auto px-6 py-6">
          <div className="flex items-center justify-between mb-4">
            {steps.map((stepItem) => {
              ...
              const isActive = step === stepItem.id || (step === 3 && stepItem.id === 4)
              const isCompleted = step > stepItem.id || (step === 3 && stepItem.id <= 4)

              return (
                <div key={stepItem.id} className="flex flex-col items-center">
                  <div
                    className={`w-12 h-12 rounded-full flex items-center justify-center mb-2 transition-colors ${
                      isActive || isCompleted ? "bg-slate-700 text-white" : "bg-gray-200 text-gray-400"
                    }`}
                  >
                    <Icon size={20} />
                  </div>
                  <span
                    className={`text-sm font-medium ${isActive || isCompleted ? "text-slate-700" : "text-gray-400"}`}
                  >
                    {stepItem.label}
                  </span>
                </div>
        ...
        {step === 1 && (
          <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-2">Оюутны ID-г оруулна уу</h2>
              <p className="text-gray-600">Ирцээ баталгаажуулахын тулд оюутны ID-г оруулна уу</p>
            </div>

            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Оюутны ID</label>
                <input
                  type="text"
                  value={studentId}
            ...
            <div className="relative mb-6">
              <div className="w-64 h-64 mx-auto relative">
                ...
                <div className="absolute inset-6 rounded-full overflow-hidden bg-gray-100 flex items-center justify-center">
                  ...
                  {!streamRef.current && <Camera size={48} className="text-gray-400" />}
                </div>
              </div>
            </div>
            ...
            {isRecognizing && (
              <div className="mb-4">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-medium text-gray-700">Таних явц</span>
                  <span className="text-sm font-medium text-gray-900">{recognitionProgress}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
        ...
        {step === 3 && (
          <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md">
            <div className="text-center mb-6">
              ...
              <h2 className="text-2xl font-bold text-gray-900 mb-2">Ирц амжилттай бүртгэгдлээ!</h2>
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="flex justify-between items-center py-2">
                ...
                <span className="bg-green-100 text-green-800 px-3 py-1 rounded-full text-sm font-medium">Батлагдсан</span>
              </div>
            </div>
